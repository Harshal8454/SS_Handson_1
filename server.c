...
void connection_handler(int connectionFileDescriptor)
{
    ...
    if (writeBytes == -1) {
        perror("❌ Error while sending the first prompt to the user!");
    } else {
        ...
        if (readBytes == -1) {
            ...
            printf("⚠️ No data was sent by the client!\n");
        } else {
            userChoice = atoi(readBuffer);  // Convert the user's choice to an integer
            switch (userChoice) {
                case 1:
